...
<div class="portlet light">
    <div class="portlet-title">
        <div class="caption font-green">
            <i class="icon-doc font-green"></i>Daftar Resensi
        </div>
        <div class="actions">
            <div class="btn-group">
                <a class="btn btn-sm btn-info" href="{{url('panel/resensi/create')}}"> <i class="icon-plus"></i> Resensi
                </a>
            </div>
        </div>
    </div>    
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover table-header-fixed" id="sample_1">
                ...
                @foreach($resensi as $resensis)
                    <tr>
                        <td> {{$x++}} </td>
                        <td> {{$resensis->judul}} </td>
                        <td> {{$resensis->status}} </td>
                        <td> {{$resensis->user->name}} </td>
                        <td> {{$resensis->tgl_buat}} </td>
                        <td> 
                            <a class="btn btn-xs btn-warning" href="{{url('/panel/resensi/'.$resensis->id.'/edit')}}"><i class="icon-note"></i></a>
                             <button type="button" class="btn btn-xs btn-danger" data-toggle="modal"

                             data-frm="<form id='hapus-resensi-{{$resensis->id}}' action='{{url('/panel/resensi/'.$resensis->id)}}' method='POST'><input type='hidden' name='_token' value='{{ csrf_token() }}'><input type='hidden' name='_method' value='DELETE'><button type='button' data-dismiss='modal' class='btn dark btn-outline'>Tidak</button><button type='submit' class='btn red'>Hapus</button></form>"

                             data-title="Apakah anda yakin menghapus resensi dengan judul : '{{$resensis->judul}}'?"
