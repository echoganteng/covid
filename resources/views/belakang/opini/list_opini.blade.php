...
<div class="portlet light">
    <div class="portlet-title">
        <div class="caption font-green">
            <i class="icon-doc font-green"></i>Daftar Opini
        </div>
        <div class="actions">
            <div class="btn-group">
                <a class="btn btn-sm btn-info" href="{{url('panel/opini/create')}}"> <i class="icon-plus"></i> Opini
                </a>
            </div>
        </div>
    </div>    
    <div class="portlet-body">
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover table-header-fixed" id="sample_1">
                ...
                <tbody>
                <?php $x=1; ?>
                @foreach($opini as $opinis)
                    <tr>
                        <td> {{$x++}} </td>
                        <td> {{$opinis->judul}} </td>
                        <td> {{$opinis->status}} </td>
                        <td> {{$opinis->user->name}} </td>
                        <td> {{$opinis->tgl_buat}} </td>
                        <td> 
                            <a class="btn btn-xs btn-warning" href="{{url('/panel/opini/'.$opinis->id.'/edit')}}"><i class="icon-note"></i></a>
                             <button type="button" class="btn btn-xs btn-danger" data-toggle="modal"

                             data-frm="<form id='hapus-opini-{{$opinis->id}}' action='{{url('/panel/opini/'.$opinis->id)}}' method='POST'><input type='hidden' name='_token' value='{{ csrf_token() }}'><input type='hidden' name='_method' value='DELETE'><button type='button' data-dismiss='modal' class='btn dark btn-outline'>Tidak</button><button type='submit' class='btn red'>Hapus</button></form>"

                             data-title="Apakah anda yakin menghapus opini dengan judul : '{{$opinis->judul}}'?"
